...
bool BFS::Compare(int a[][3], node *open, node *close)
{
	node *p,*q;
	p = open->next;
	q = close->next;
...
void BFS::Extend(int a[][3], int t1[][3], int t2[][3], int t3[][3], int t4[][3])
{
	int i, j,k;
	int temp[4];
	int flag=0;
...
int BFS::Algorithm(int start[][3], int target[][3])
{
	...
	while (num<10000) {
		if (openlist->next ==NULL) 
			...
			p = openlist->next; //将open表中第一个节点取出
			q = (node *)malloc(sizeof(node));
